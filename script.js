...
class Dashboard {
  ...
  async loadData() {
    ...
    this.movies = this.parseCSV(csvText).map(m=>{
      if(!m.runtime||m.runtime===0) m.runtime=100; // point 3
      return m;
    });
  ...
  setupFilter() {
    ...
    document.getElementById('clearFilter').onclick=()=>{
      this.filtered=null; this.renderDiary();
    };
  }
  ...
  renderDiary(){
    ...
    this.genChart(m,'genreChart',    x=>x.genre.split(',').map(g=>g.trim()), 'doughnut');
    this.genChart(m,'ratingChart',   x=>[parseFloat(x.rating).toFixed(1)], 'bar', {labels:Object.keys(this.stats.rating_distribution)});
    this.monthChart(m,'monthlyCountChart', 1);  // count
    this.monthChart(m,'monthlyMinutesChart',2); // minutes

    this.topList(m,'directors-year', x=>x.director, yr);
    this.topList(m,'directors-all',  x=>x.director);
    this.topList(m,'cast-year',      x=>x.cast, yr);
    this.topList(m,'cast-all',       x=>x.cast);
  ...
  genChart(m,el,fn,type,opts={}){
    const dist={};
    m.forEach(x=>fn(x).forEach(v=>{ if(v) dist[v]=(dist[v]||0)+1; }));
    const ctx=document.getElementById(el).getContext('2d');
    if(this.charts[el]) this.charts[el].destroy();
    this.charts[el]=new Chart(ctx,{type, data:{
      labels:opts.labels||Object.keys(dist),
      datasets:[{data:Object.values(dist), backgroundColor:'#00d735'}]
    }});
  }

  monthChart(m,el,mode){
    const arr=Array(12).fill(0), y=new Date().getFullYear();
    m.forEach(x=>{
      if(x.watch_date.startsWith(y)){
        const mo=new Date(x.watch_date).getMonth();
        arr[mo]+= mode===1?1:x.runtime;
      }
    });
    const ctx=document.getElementById(el).getContext('2d');
    if(this.charts[el]) this.charts[el].destroy();
    this.charts[el]=new Chart(ctx,{type:'line', data:{
      labels:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],
      datasets:[{data:arr,borderColor:'#00d735', backgroundColor:'rgba(0,215,53,0.2)', fill:true}]
    }});
  }

  topList(m,el,fn,year){
    const cnt={}, arr=m.filter(x=>!year||x.watch_date.startsWith(year)).flatMap(x=>fn(x));
  ...
  renderStats(){
    ...
    `).join('');
    // genre breakdown
    this.genChart(m,'genre-breakdown', x=>x.genre.split(',').map(g=>g.trim()), 'bar');
    // country count
    this.genChart(m,'country-count', x=>[x.country], 'bar');
  }

  // ---- List Tab ----
  renderList(){
    const m=this.getMovies(), yr=new Date().getFullYear();
    const tr=(films)=>films.sort((a,b)=>b.rating-a.rating).slice(0,5)
    const yt=tr(m.filter(x=>+x.rating>=0 && x.watch_date.startsWith(yr)));
    const all=tr(m.filter(x=>+x.rating>=0));
    document.getElementById('top-rated-year').innerHTML=yt.map(x=>`<div>${x.title} (${x.rating}★)</div>`).join('');
